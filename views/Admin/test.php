...
<head>
    ...
    <script type="text/javascript" src="__PUBLIC__/jquery-easyui/locale/easyui-lang-zh_TW.js"></script>
</head>
<body>
        ...
        function destroyDate(){
            ...
            if (row){
                $.messager.confirm('Confirm','確定要刪除這筆資料嗎？',function(r){
                    if (r){
                        $.post('{:U('Busdate/remove')}',{id:row.did},function(result){
                            console.log(result);
                            if (result == 1){
                                $('#dg').datagrid('reload'); 
        ...
        function destroyAll(){
            var row = $('#dg').datagrid('getSelected');
                ...
                $.messager.confirm('Confirm','確定要刪除今日之前全部班次嗎？',function(r){
                    if (r){
                        $.post('{:U('Busdate/removeAll')}',function(result){
                            console.log(result);
                            if (result == 1){
                                $('#dg').datagrid('reload'); 
                                  // reload the user data
